...
Remember: the objective is to allow the programmer to activate better code paths once the program discovered or proved their objects satisfy the needed guarantees.  And we want to avoid copy and pasting<sup>[1: copy and pasting is evil](#copy-and-pasting-is-evil)</sup>.
...
But what would mean two iterators are disjoint? that the dereferenced value does not alias and that the `iterator++` gives you an iterator that is also disjoint.
